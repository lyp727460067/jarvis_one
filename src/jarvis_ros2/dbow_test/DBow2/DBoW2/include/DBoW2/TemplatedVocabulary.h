...
{		
public:
//  friend std::unique_ptr<BriefVocabulary> Dbow2FromProto(
//      const proto::VocabularyData &proto, const proto::VocabularyOption &option);
//  friend proto::VocabularyData Dbow2ToProto(const std::string &name,
//                                                   const BriefVocabulary &voc);
 ...
 TemplatedVocabulary(const std::string &filename, bool);
  ...
  * @param filename
  */
 bool loadFromTextBin(const std::string &filename);
 void loadBin(const std::string &filename);
 /**
  * Saves the vocabulary into a text file
  * @param filename
  */
 void saveToTextBin(const std::string &filename) const;
...
template<class TDescriptor, class F>
bool TemplatedVocabulary<TDescriptor, F>::loadFromTextBin(
...
template <class TDescriptor, class F>
void TemplatedVocabulary<TDescriptor, F>::saveToTextBin(
...
void TemplatedVocabulary<TDescriptor, F>::loadBin(const std::string &filename) {
  ...
  CHECK(ifStream.good())<<"open file faild with file name:"<<filename;
  ...
  for (unsigned int i = 0; i < voc.nNodes; ++i) {
    ...
    //有可能会系统的位数不一样出错
    // m_nodes[nid].descriptor = std::bitset<256>(
    //     voc.nodes[i].descriptor, voc.nodes[i].descriptor + 4);
    // boost::dynamic_bitset test = boost::dynamic_bitset<>(
    //     voc.nodes[i].descriptor, voc.nodes[i].descriptor + 4);

    auto ToDescriptror = [](const uint64_t *descriptor) {
      std::bitset<256> result;
      int j = 0;
      for (int index = 0; index < 4; index++) {
        const uint64_t &byte = descriptor[index];
        for (int i = 0; i < 64; i++) {
          if (byte & ( uint64_t(0x0000000000000001) << i)) {
            result.set(j);
          } else {
            result.reset(j);
          }
    ...
    m_nodes[nid].descriptor = ToDescriptror(voc.nodes[i].descriptor);
    // for (int i = 0; i < 256; i++) {
    //   CHECK_EQ(m_nodes[nid].descriptor[i],test[i]);
    // }
    // std::cout<<std::endl; 
    // CHECK(false);
    if (i < 5) {
      std::string test = m_nodes[nid].descriptor.to_string();
      // std::to_string(m_nodes[nid].descriptor, test);
