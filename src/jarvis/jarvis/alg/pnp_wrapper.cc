...
std::pair<transform::Rigid3d, std::set<FeatureId>> CalculatePoseUsingOpencvPnP(
  ...
  CHECK_EQ(map_points.size(), 4);
  CHECK_EQ(features.size(), 4);
  ...
  for (const auto& map_point : map_points) {
    if (map_point.first.sequence_id == 0 && features.count(map_point.first)) {
  ...
  solvePnPRansac(map_points_temp, key_points_temp, K, D, rvec, t, true, 200,
  ...
  cv::cv2eigen(t, T_pnp);
  return_result.first = transform::Rigid3d(T_pnp, Eigen::Quaterniond(R_pnp));
...
std::pair<transform::Rigid3d, std::set<FeatureId>> CalculatePoseUsingOrbslamPnP(
  ...
  return_result.first = pnp_pose.first;
