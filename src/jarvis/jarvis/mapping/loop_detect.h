...
struct LoopDetectOption {
  ...
  int continu_min_pnp_inliers_num = 20;
...
class LoopDetect {
 public:
  ...
  void Detect(const std::pair<LocalMapId, std::shared_ptr<LocalMap>>& local_map,
              ...
              const double min_score, bool continue_find=false);
  void WhenDone(
  ...
  //
  std::vector<std::unique_ptr<LoopDetctResult>> DetectForConinue(
      std::shared_ptr<LocalMap> local_map, KeyFrameId kf_id,
      const std::map<KeyFrameId, KeyFrameData>& kf_datas, const KeyFrameDataBase* data_base,
      std::unique_ptr<ConstraintConsistentFilter>* consistent_filte,
      const double min_score);
  //
  void UpdateDataBase(
      const std::map<LocalMapId, std::shared_ptr<LocalMap>>& local_map);
  //
  const KeyFrameDataBase* GetDataBase(const LocalMapId& id) {
    CHECK(key_frame_data_base_.count(id));
    return key_frame_data_base_.at(id).get();
  }

 private:
  //

  std::map<KeyFrameId, transform::Rigid3d> ComputeContinueConstraint(
  ...
  ComputeConstraintUsingPnP(std::shared_ptr<LocalMap> local_map,
