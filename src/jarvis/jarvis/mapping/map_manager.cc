...
MapManager::MapManager(const MapManagerOption &option,
  ...
  //
  global_loop_detect_kf_sampler_ = std::make_unique<common::FixedRatioSampler>(
      options_.global_constraint_compute_sampler);
  //
  //
...
void MapManager::ComputeLoopConstaint(const LocalMapId &local_map_id,
                                      ...
                                      const double min_score) {
  //
  bool need_find_constraints_ =false;
  bool constinue_find = false;
  auto last_connection_time = trajectory_connectivity_state_.LastConnectionTime(
      key_frame_id.trajectory_id, local_map_id.trajectory_id);
  //
  ...
  if (key_frame_id.trajectory_id == local_map_id.trajectory_id) {
    if (loop_detect_kf_sampler_->Pulse()) {
      ...
      //     kf_data.data->pose;
        need_find_constraints_ = true;
    }
  }
  //
  if (key_frame_id.trajectory_id != local_map_id.trajectory_id) {
    // if (continue_loop_count_++ > 3) {
    //   continue_loop_count_ = 0;
      if (kf_time >
              last_connection_time +
                  common::FromSeconds(
                      options_.global_constraint_search_after_n_seconds) ||
          global_loop_detect_kf_sampler_->Pulse()) {
        need_find_constraints_ = true;
        constinue_find = true;
      }
    // }
  ...
  //
  if (!need_find_constraints_) return;
  ...
  if (continuous_ids.size() < options_.continuous_candidate_loop_frame / 2)
    return;
  if (constinue_find) {
    ...
    loop_detect_->Detect(
        ...
        continuous_ids, min_score, constinue_find);
    return;
  }
  LOG(INFO) << Tag << "LoopDetect  -> local_map" << local_map_id
            << ",keyframeid " << key_frame_id << log_info::RESET;
...
void MapManager::ExtendedKeyFrameData(const LocalMap &local_map,
                                      ...
                                      std::function<void()> need_relocation) {
  if (!enable_loop_closure_) return;
  //
  trajectory_connectivity_state_.Add(id.trajectory_id);
  //
  ...
  *local_map_temp = local_map;
  work_item_queue_->AddWorkItem([=]() {
    estimator::TicToc cost_tictoc;
    KeyFrameData data = data_temp;
    std::set<MapPointId> exist_map_point_ids;
    for (const auto &id : data.data->map_point_ids) {
      exist_map_point_ids.insert(id.second);
    }
    auto result_match =
        local_map_track_->Track(local_map_temp, data, exist_map_point_ids);
    //
    if (result_match) {
    ...
    //
    VLOG(0) << "ExtendedKeyFrameData cost:" << cost_tictoc.toc() << " ms";
...
std::shared_ptr<LocalMap> MapManager::ReconstructLocalMap(
  ...
  for (const auto &kf_data : local_map->AllKeyFrameDatas()) {
    ...
    if (previous_local_map_updated_key_frames_id_.count(kf_data.id) == 0) {
      map_point_construct_->ConstructExtend(*new_local_map_ptr, data.data.get(),
                                            &connect_data);
    }
    //
    new_update_ids.insert(kf_data.id);
  ...
  previous_local_map_updated_key_frames_id_ = std::move(new_update_ids);
  VLOG(0) << "ReconstructLocalMap cost:" << tictoc1.toc() << " ms";
  //
...
//
void MapManager::AddLocalMap(
    int trajectory, std::shared_ptr<LocalMap> local_map,
    std::function<void(const LocalMapId &, std::shared_ptr<LocalMap>)>
        update_new_local_map) {
  if (!enable_loop_closure_) return;
  work_item_queue_->AddWorkItem([&, local_map, trajectory,update_new_local_map]() {
    estimator::TicToc add_local_map_tictoc;
    ...
    //
    estimator::TicToc add_local_map_tictoc1;
    new_local_map->UpdadataExtendFinishData(true);
    VLOG(0) << "UpdadataExtendFinishData cost:" << add_local_map_tictoc1.toc()
            << " ms";
    UpdateKeyframeDataUsingPrunedLocalMap(local_map);
    //
    // if (!local_maps_.SizeOfTrajectoryOrZero(trajectory) &&
    //     update_new_local_map) {
    //   LOG(INFO)<<local_maps_.size()<<" "<<trajectory;
    //   auto last_local_map =
    //       local_maps_.EndOfTrajectory(trajectory)->data.local_map;
    //   auto last_local_map_id = local_maps_.EndOfTrajectory(trajectory)->id;
    //   update_new_local_map(last_local_map_id, last_local_map);
    // }
    ...
    //
    VLOG(0) << "AddLocalMap cost:" << add_local_map_tictoc.toc() << " ms";

    new_local_map_id_ = local_map_id;
    //
    loop_detect_->UpdateDataBase(
        std::map<LocalMapId, std::shared_ptr<LocalMap>>{
            {local_map_id, new_local_map}});
    //
    if (update_new_local_map) {
      update_new_local_map(local_map_id, new_local_map);
    }
    return WorkItem::Result::Normal;
  });
  //
...
KeyFrameId MapManager::AddKeyFrameData(int trajectory,
  ...
  std::lock_guard<std::mutex> lock(mutex_);
  if (key_frames_datas_.empty()) {
    trajectory_connectivity_state_.Connect(trajectory, trajectory,
                                           data.data->time);
  }
...
void MapManager::AddLoopConstraints(
  ...
  for (auto &&l_constraint : loop_constraint) {
    //
    common::Time time = key_frames_datas_.at(l_constraint->kf_id).data->time;
    trajectory_connectivity_state_.Connect(
        l_constraint->kf_id.trajectory_id,
        l_constraint->local_map_id.trajectory_id, time);
    LOG(INFO) << "Find constraints: " << "local_map "
              << l_constraint->local_map_id << "-" << l_constraint->in_map_kf_id
...
void MapManager::Optimization(
    std::vector<std::unique_ptr<LoopDetctResult>> &&loop_constraint) {
  AddLoopConstraints(std::move(loop_constraint));
  //
  ...
  local_to_global_transform_ =
      key_frames_datas_.at(last_key_frame_id).global_pose *
      key_frames_datas_.at(last_key_frame_id).data->pose.inverse();
  {
    std::lock_guard<std::mutex> lock(mutex_);
