...
void LoopDetect::UpdateDataBase(
    const std::map<LocalMapId, std::shared_ptr<LocalMap>>& local_maps) {
  std::map<LocalMapId, std::unique_ptr<KeyFrameDataBase>> data_bases;
  for (const auto& local_map : local_maps) {
    if (key_frame_data_base_.count(local_map.first)) continue;
    //
    data_bases[local_map.first] =
        std::make_unique<KeyFrameDataBase>(options_.key_frame_data_option);
    for (const auto& data : local_map.second->ConstData().key_frames_datas) {
      data_bases[local_map.first]->AddData(data.id, data.data.data);
    }
  }
  std::lock_guard<std::mutex> lock(mutex_);
  for (auto& data_base : data_bases) {
    key_frame_data_base_[data_base.first] = std::move(data_base.second);
  }
}
...
void LoopDetect::Detect(
    ...
    const double min_score,bool continue_find) {
  //
  ...
  detect_node_task->SetWorkItem([=]() {
    ...
    if (!continue_find) {
      for (auto kf_data : kf_datas) {
        // CHECK((*this_kf_result_catch_ptr) ==
        // nullptr)<<this_kf_result_catch_ptr;
        auto loop_result =
      ...
      for (auto kf_data : kf_datas) {
        ...
        // nullptr)<<this_kf_result_catch_ptr;
        auto loop_results =
            DetectForConinue(local_map.second, kf_data.first, kf_datas,
                             key_frame_data_base_[local_map.first].get(),
                             &consistent_filter, min_score);
...
std::unique_ptr<LoopDetctResult> LoopDetect::DetectForOne(
  ...
  for (auto const& id : best_candidata_kfs) {
    (*consistent_filter)
        ->Update(
            id.first,
            local_map->ConstData().covisibility.GetConnectedKeyFrames(id.first),
            kf_id);
    break;
  }
  ...
  for (const auto& candidata_kf : filter_candidate_ids) {
    ...
    auto constraint =
        ComputeConstraint(local_map, candidata_kf.first, kf_id, kf_data);
...
//
std::vector<std::unique_ptr<LoopDetctResult>> LoopDetect::DetectForConinue(
    std::shared_ptr<LocalMap> local_map, KeyFrameId kf_id,
    const std::map<KeyFrameId, KeyFrameData>& kf_datas,
    const KeyFrameDataBase* data_base,
    std::unique_ptr<ConstraintConsistentFilter>* consistent_filter,
    const double min_score) {
  const auto candidate_kfs = data_base->FindSimilarCandidate(
      kf_datas.at(kf_id).data, NotNeedToDetectKf(local_map), min_score);
  // //
  auto const best_candidata_kfs =candidate_kfs;
      // FilterBestDbowResultWithCovisibility(local_map, candidate_kfs);
  if (best_candidata_kfs.empty()) {
    (*consistent_filter) = std::make_unique<ConstraintConsistentFilter>(
        options_.constraint_consistent_filter_num);
  ...
  std::map<KeyFrameId, std::tuple<transform::Rigid3d,
                                  std::vector<std::pair<FeatureId, MapPointId>>,
                                  KeyFrameData>>
      continue_constraints;
  std::map<KeyFrameId, KeyFrameId> target_candidate_ids;
  for (const auto& coninue_ids : filter_candidate_ids) {
    //
    //
    for (const auto& coninue_ids_traget : coninue_ids.second) {
      for (const auto candidata_kf : coninue_ids_traget.second) {
        auto constraint = ComputeConstraintUsingPnP(
            local_map, candidata_kf, coninue_ids_traget.first,
            kf_datas.at(coninue_ids_traget.first));
        //
        if (!constraint.second.empty()) {
          continue_constraints.emplace(
              coninue_ids_traget.first,
              std::make_tuple(constraint.first, constraint.second,
                              kf_datas.at(coninue_ids_traget.first)));
          target_candidate_ids.emplace(coninue_ids_traget.first, candidata_kf);
          break;
        }
      }
    }
  ...
  if (continue_constraints.size() >
      options_.constraint_consistent_filter_num - 2) {
    *consistent_filter = std::make_unique<ConstraintConsistentFilter>(
        options_.constraint_consistent_filter_num);
    auto constraint_poses =
        ComputeContinueConstraint(local_map, continue_constraints);
    std::stringstream info;
    std::vector<std::unique_ptr<LoopDetctResult>> result;
    for (const auto& constraint_pose : constraint_poses) {
      //
      const KeyFrameId candidate_id =
          target_candidate_ids.at(constraint_pose.first);
      auto& candidate_kf_data = local_map->AllKeyFrameDatas().at(candidate_id);
      const transform::Rigid3d pnp_pose =
          std::get<0>(continue_constraints.at(constraint_pose.first));
      const transform::Rigid3d target_pose =
          kf_datas.at(constraint_pose.first).data->pose;
      const transform::Rigid3d candidata_pose = candidate_kf_data.data->pose;
      //

      result.emplace_back(std::make_unique<LoopDetctResult>(
          LoopDetctResult{constraint_pose.first,
                          {-1, 0},
                          constraint_pose.second,
                          candidate_id}));

      info << " target " << constraint_pose.first
           << "candidate: " << candidate_id << " pnp pose "
           << std::get<0>(continue_constraints.at(constraint_pose.first))
           << "optimization " << constraint_pose.second;
      //
      info << "local pose:  " << candidate_kf_data.data->pose
           << local_map->LocalPose() * pnp_pose << ","
           << local_map->LocalPose() *  constraint_pose.second
...
LoopDetect::ComputePnpPose(std::shared_ptr<LocalMap> local_map,
  ...
  if (inlier_pairs.size() < options_.continu_min_pnp_inliers_num) {
    LOG(WARNING) << "pnp inli size: " << inlier_pairs.size() << " LE "
                 << options_.continu_min_pnp_inliers_num;
    return {};
  }
...
std::unique_ptr<LoopDetctResult> LoopDetect::ComputeConstraint(
  ...
  const transform::Rigid3d imu_pose_temp =
      target_kf_data.data->ImuPose(pnp_pose.first, 0);

  auto ypr = transform::Rot2ypr(imu_pose_temp.rotation().toRotationMatrix()) *
             M_PI / 180.;
  auto ypr1 = transform::Rot2ypr(
                  target_kf_data.data->pose.rotation().toRotationMatrix()) *
              M_PI / 180.;
  const transform::Rigid3d imu_pose =
      transform::Rigid3d(imu_pose_temp.translation(),
                         transform::RollPitchYaw(ypr1[2], ypr1[1], ypr[0]));
  ...
  inliner = RemoveOutliersRejection(target_kf_data.data->extric_camera_to_imu,
                                    ...
                                    options_.outlier_min_err * 2 / 460,
  ...
  //
  // CHECK(abs(delta_pose.translation().z()) < 0.2);
  if (delta_pose.translation().norm() > options_.constraint_max_distance ||
      abs(delta_pose.translation().z()) > 2.0) {
  ...
  info << local_map->LocalPose() * init_pose << ","
       << local_map->LocalPose() * pnp_imu_pose << ","
       << " target: " << target_pose << " candidate: " << candidata_pose;
  info << "delta pose :" << delta_pose << " yaw :" << delta_yaw;
  LOG(INFO) << Tag << info.str() << log_info::RESET;
  transform::Rigid3d err_pose =
      (target_pose.inverse() * local_map->LocalPose() * init_pose);
  // CHECK(err_pose.translation().transpose().norm() < 0.1);
...
//
std::pair<transform::Rigid3d, std::vector<std::pair<FeatureId, MapPointId>>>
LoopDetect::ComputeConstraintUsingPnP(std::shared_ptr<LocalMap> local_map,
                                      const KeyFrameId& candidate_id,
                                      const KeyFrameId& target_id,
  ...
  //
  //
  auto ypr = transform::ToRollPitchYaw(imu_pose_temp.rotation());
  auto ypr1 = transform::ToRollPitchYaw(target_kf_data.data->pose.rotation());
  //
  const transform::Rigid3d imu_pose =
      transform::Rigid3d(imu_pose_temp.translation(),
                         transform::RollPitchYaw(ypr1[0], ypr1[1], ypr[2]));
  const transform::Rigid3d pnp_imu_pose = imu_pose;
  KeyFrameMapPointsDataWithFeatIds canditate_map_points;
  //
  auto frame_feat_map_points_ids =
  ...
  //
  if (candidate_additional_map_points_ids.size() < 10) return {};
  std::map<MapPointId, Eigen::Vector3d> candidate_additional_map_points_datas;
  //
  for (const auto& map_point_data : local_map->AllMapPoints()) {
    candidate_additional_map_points_datas.emplace(
  ...
  inliner = RemoveOutliersRejection(target_kf_data.data->extric_camera_to_imu,
                                    candidate_additional_map_points_datas,
                                    target_kf_data.data->features, init_pose,
                                    options_.outlier_min_err * 1 / 460,
                                    candidate_additional_map_points_ids);
  return {imu_pose, candidate_additional_map_points_ids};
  //
}

std::map<KeyFrameId, transform::Rigid3d>
LoopDetect::ComputeContinueConstraint(
    std::shared_ptr<LocalMap> local_map,
 
    std::map<
        KeyFrameId,
        std::tuple<transform::Rigid3d,
                   std::vector<std::pair<FeatureId, MapPointId>>, KeyFrameData>>
        constraints) {
  //
  ...
  std::map<KeyFrameId, NodeData> ceres_datas;
  //
  for (const auto& constraint : constraints) {
    auto const pose = std::get<0>(constraint.second);
    Eigen::Vector3d rpy = transform::ToRollPitchYaw(pose.rotation());
    ceres_datas.emplace(
        constraint.first,
        NodeData{
            pose.rotation(), std::array<double, 3>{rpy.x(), rpy.y(), rpy.z()},
            pose.translation(), std::get<2>(constraint.second).data->pose});
    problem.AddParameterBlock(ceres_datas.at(constraint.first).t.data(), 3);
    problem.AddParameterBlock(&ceres_datas.at(constraint.first).rpy[2], 1);
  }
  //
  //
  //
  const std::vector<transform::Rigid3d> extric_camera_to_imu =
      std::get<2>(constraints.begin()->second).data->extric_camera_to_imu;
  //
  //
  ...
  for (size_t i = 0; i < options_.track_sequence.size(); i++) {
    ...
    transform::Rigid3d extir_iverse =
        extric_camera_to_imu[options_.track_sequence[i][0]];
  ...
  //
  const auto all_map_points = local_map->AllMapPoints();
  for (const auto constraint : constraints) {
    auto const matched_ids = std::get<1>(constraint.second);

    auto const target_kf_data = std::get<2>(constraint.second);
    for (const auto& match_id : matched_ids) {
      const Eigen::Vector3d& mp_pos =
          all_map_points.at(match_id.second).data->pos;
      auto const& nomal_point =
          target_kf_data.data->features.at(match_id.first);
      problem.AddResidualBlock(
          FourReProjectionErrNormal::Creat(nomal_point.f, mp_pos,
                                       ceres_datas.at(constraint.first).rpy[0],
                                       ceres_datas.at(constraint.first).rpy[1],
                                       options_.op_weight),
          nullptr, ceres_datas.at(constraint.first).t.data(),
          &ceres_datas.at(constraint.first).rpy[2],
  ...
  //
  auto first_it = constraints.begin();
  for (auto it = std::next(constraints.begin());
       it != constraints.end(); ++it) {
    //
    auto const pose0 = std::get<2>(first_it->second).data->pose;
    auto const pose1 = std::get<2>(it->second).data->pose;
    const double delta_yaw =
        NormalizeAngle(transform::GetYaw(pose1) - transform::GetYaw(pose0));
    const auto delta_translation = (pose0.inverse() * pose1).translation();
    //
    ceres::CostFunction* cost_function = FourRePoseGraphErr::Creat(
        delta_translation, delta_yaw, ceres_datas.at(it->first).rpy[0],
        ceres_datas.at(first_it->first).rpy[1], 1e4);
    problem.AddResidualBlock(
        cost_function, nullptr, ceres_datas.at(first_it->first).t.data(),
        &ceres_datas.at(first_it->first).rpy[2],
        ceres_datas.at(it->first).t.data(), &ceres_datas.at(it->first).rpy[2]);
    first_it = it;
  };
  //
  ceres::Solver::Options options;
  options.minimizer_progress_to_stdout = false;
  options.max_num_iterations = 5;  // options_.max_num_iterations;
  options.linear_solver_type = ceres::SPARSE_NORMAL_CHOLESKY;
  ceres::Solver::Summary summary;
  ceres::Solve(options, &problem, &summary);
  //
  std::map<KeyFrameId, transform::Rigid3d> result;
  for (const auto& ceres_data : ceres_datas) {
    result.emplace(
        ceres_data.first,
        transform::Rigid3d(ceres_data.second.t,
                           transform::RollPitchYaw(ceres_data.second.rpy[0],
                                                   ceres_data.second.rpy[1],
                                                   ceres_data.second.rpy[2])));
  }
  return std::move(result);
}
transform::Rigid3d LoopDetect::Optimize(
  ...
  auto ve_rpy = transform::ToRollPitchYaw(rotation);

  std::array<double, 3> rpy{ve_rpy[0], ve_rpy[1], ve_rpy[2]};

  //
  problem.AddParameterBlock(traslation.data(), 3);
  problem.AddParameterBlock(&rpy[2], 1);
  ...
  for (const auto& match_id : matched_ids) {
    ...
    problem.AddResidualBlock(
        FourReProjectionErrNormal::Creat(nomal_point.f, mp_pos,
                                         rpy[0], rpy[1], weight[0]),
       nullptr, traslation.data(), &rpy[2],
        ex_traslation[match_id.first.sequence_id].data(),
        ex_rotation[match_id.first.sequence_id].coeffs().data());
    //
  }
  ...
  ceres::Solver::Options options;
  options.minimizer_progress_to_stdout = false;
  options.max_num_iterations = 2;  // options_.max_num_iterations;
  options.linear_solver_type = ceres::SPARSE_NORMAL_CHOLESKY;
  ceres::Solver::Summary summary;
  ceres::Solve(options, &problem, &summary);
  ...
  return transform::Rigid3d(traslation,
                            transform::RollPitchYaw(rpy[0], rpy[1], rpy[2]));
}
