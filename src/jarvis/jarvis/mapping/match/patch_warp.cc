...
void getWarpMatrixAffineHomography(
  ...
  const Position t = T_cur_ref.inverse().translation();
  const Eigen::Vector3d xyz_ref(f_ref * depth_ref);
  const Eigen::Matrix3d H_cur_ref =
      T_cur_ref.rotation().toRotationMatrix() *
  ...
  const int kHalfPatchSize = 5;
  const Eigen::Vector3d f_cur(H_cur_ref * f_ref);
  Eigen::Vector3d f_du_ref, f_dv_ref;
  cam_ref->backProject3(
      px_ref + Eigen::Vector2d(kHalfPatchSize, 0) * (1 << level_ref),
      &f_du_ref);
  cam_ref->backProject3(
  ...
  for (int y = -halfpatch_size; y < halfpatch_size; ++y) {
    for (int x = -halfpatch_size; x < halfpatch_size; ++x, ++patch_ptr) {
      ...
      if (xi < 0 || yi < 0 || xi + 1 >= img_ref.cols || yi + 1 >= img_ref.rows)
        ...
        *patch_ptr = static_cast<uint8_t>(w00 * ptr[0] + w01 * ptr[stride] +
...
bool warpPixelwise(const Frame& cur_frame, const Frame& ref_frame,
  ...
  Eigen::Vector3d xyz_cur = cur_frame.pose.inverse() * ref_frame.pose * xyz_ref;
  ...
  uint8_t* patch_ptr = patch;
  const cv::Mat& img_ref = ref_frame.img_pyr[level_ref];
  const int stride = img_ref.step.p[0];
  ...
  for (int y = -halfpatch_size; y < halfpatch_size; ++y) {
    for (int x = -halfpatch_size; x < halfpatch_size; ++x, ++patch_ptr) {
      ...
      Eigen::Vector3d ele_xyz_ref =
          ref_frame.pose.inverse() * (cur_frame.pose) * ele_xyz_cur;
      //
...
void createPatchNoWarp(const cv::Mat& img, const Eigen::Vector2i& px,
  ...
  CHECK(px(0) >= halfpatch_size && px(1) >= halfpatch_size &&
        px(0) < img.cols - halfpatch_size && px(1) < img.rows - halfpatch_size);
...
void createPatchNoWarpInterpolated(const cv::Mat& img,
  ...
  uint8_t* patch_ptr = patch;
  for (int y = 0; y < patch_size; ++y) {
    uint8_t* img_ptr = (uint8_t*)img.data + (v_r - halfpatch_size + y) * step +
                       (u_r - halfpatch_size);
    for (int x = 0; x < patch_size; ++x, ++patch_ptr, ++img_ptr) {
      *patch_ptr = wTL * img_ptr[0] + wTR * img_ptr[1] + wBL * img_ptr[step] +
                   wBR * img_ptr[step + 1];
      ;
    }
  }
