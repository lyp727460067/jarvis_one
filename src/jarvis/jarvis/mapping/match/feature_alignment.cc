...
bool align1D(const cv::Mat& cur_img,
             ...
             const int n_iter, const bool affine_est_offset,
  ...
  for (int y = 0; y < kPatchSize; ++y) {
    ...
    for (int x = 0; x < kPatchSize; ++x, ++it, ++it_dv) {
      ...
      const float dy =
          static_cast<float>(it[ref_step]) - static_cast<float>(it[-ref_step]);
  ...
  for (int iter = 0; iter < n_iter; ++iter) {
    ...
    if (std::isnan(u) ||
        std::isnan(v))  // TODO very rarely this can happen, maybe H is
    ...
    for (int y = 0; y < kPatchSize; ++y) {
      ...
      for (int x = 0; x < kPatchSize; ++x, ++it, ++it_ref, ++it_ref_dv) {
        float cur_intensity = wTL * it[0] + wTR * it[1] + wBL * it[cur_step] +
                              wBR * it[cur_step + 1];
...
bool align2D_SSE2(const cv::Mat& cur_img, uint8_t* ref_patch_with_border,
  ...
  for (int iter = 0; iter < n_iter; ++iter) {
    ...
    update_u = ((A12 * b2 - A22 * b1) * Dinv) *
               2;  // * 2 to compensate because above, we did not compute the
...
bool align2D_NEON(const cv::Mat& cur_img, uint8_t* ref_patch_with_border,
  ...
  for (int iter = 0; iter < n_iter; ++iter) {
    ...
    for (int y = 0; y < patch_size; ++y) {
      ...
      // Eigen::Vector multiply accumulate long: vmla -> Vr[i] := Va[i] + Vb[i]
      // * Vc[i] int32x4_t  vmlal_s16(int32x4_t a, int16x4_t b, int16x4_t c); //
      // VMLAL.S16 q0,d0,d0
...
void alignPyr2DVec(const std::vector<cv::Mat>& img_pyr_ref,
                   ...
                   const int min_level, const std::vector<int>& patch_sizes,
  ...
  for (size_t i = 0; i < px_ref.size(); ++i) {
    ...
    bool res =
        alignPyr2D(img_pyr_ref, img_pyr_cur, max_level, min_level, patch_sizes,
  ...
  for (int level = max_level; level >= min_level; --level) {
    ...
    for (int y = 0; y < patch_size; ++y) {
      ...
      for (int x = 0; x < patch_size; ++x, ++it, ++it_patch, ++it_dx, ++it_dy) {
        ...
        *it_dx = static_cast<int16_t>(it[1]) - it[-1];
        *it_dy =
            static_cast<int16_t>(it[step]) - it[-step];  // divide by 2 missing
    ...
    for (int iter = 0; iter < n_iter; ++iter) {
      ...
      if (u_r < 0 || v_r < 0 || u_r >= width - patch_size ||
        ...
        std::cout << "current patch is too close to the border "
                  ...
                  << height << "\t patch_size = " << patch_size << std::endl;
#endif
      ...
      const uint16_t wTL = static_cast<uint16_t>(
          (1.0f - subpix_x) * (1.0f - subpix_y) * (1 << SHIFT_BITS));
      const uint16_t wTR = static_cast<uint16_t>(subpix_x * (1.0f - subpix_y) *
                                                 (1 << SHIFT_BITS));
      const uint16_t wBL = static_cast<uint16_t>((1.0f - subpix_x) * subpix_y *
                                                 (1 << SHIFT_BITS));
      ...
      for (int y = 0; y < patch_size; ++y) {
        ...
        for (int x = 0; x < patch_size; x += 8, it += 8, it_ref += 8) {
          ...
          // use reinterpet-cast to make signed [-255,255]
          int16x8_t res =
              vsubq_s16(vreinterpretq_s16_u16(v00),
                        vreinterpretq_s16_u16(vmovl_u8(vld1_u8(it_ref))));

          // Vector multiply accumulate long: vmla -> Vr[i] := Va[i] + Vb[i] *
          // Vc[i] int32x4_t  vmlal_s16(int32x4_t a, int16x4_t b, int16x4_t c);
          // // VMLAL.S16 q0,d0,d0
      ...
      for (int y = 0; y < patch_size; ++y) {
        ...
        for (int x = 0; x < patch_size;
          ...
          uint16_t cur = SVO_DESCALE(
              wTL * it[0] + wTR * it[1] + wBL * it[step] + wBR * it[step + 1],
              SHIFT_BITS);
      ...
      update = Hinv * Jres * 2.0;  // * 2 to compensate because above, we did
    ...
    if (!converged && !go_to_next_level)
      return false;  // no need to go to next level when we didn't converge at
                     // this one.
