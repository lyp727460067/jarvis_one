...
std::map<std::string, std::string> map_to_name{
    ...
    {"/home/lyp/data/test_0045/1719/jarvis/data/2024_10_30_16_56",
     "kTestPictureDataDir_2024_10_30_16_56"}};

constexpr char kTestPictureDataDirTarget[] =
    "/home/lyp/data/test_0045/1719/jarvis/data/2024_10_30_17_9";
...
class KeyFrameDatabaseTest : public ::testing::Test {
 public:
  virtual void SetUp() {
    ...
    data_base_ =
        std::make_unique<KeyFrameDataBase>(KeyFrameDataBaseOption{}, voc_);
    des_extractor_ =
...
std::shared_ptr<const KeyFrameData::Data> KeyFrameDatabaseTest::ComuteData(
  ...
  for (int i = 0; i < images.size(); i++) {
    std::vector<cv::KeyPoint> key_points = key_extractor_->Extract(images[i]);
    if(key_points.empty())continue;
    Descriptors descriptors = des_extractor_->Extract(images[i], key_points);
    for (int j = 0; j < key_points.size(); j++) {
      const FeatureId feat_id(i, j);
...
TEST_F(KeyFrameDatabaseTest, FindCanditate) {
  ...
  for (auto dir : map_to_name) {
    ...
    int index = 0;
    common::FixedRatioSampler sampler(0.01);
  ...
  auto result = data_base_->FindSimilarCandidate(target, exclude_ids);
  for (auto const& r : result) {
    LOG(INFO) << "result  :" << r.first << " score: " << r.second
              << " name:" << id_map_name[r.first];
  }
  //
  auto image_datas =
