...
void GrowAsNeeded(const sensor::RangeData& range_data,
  ...
  if (!range_data.sector.end_points.empty()) {
    bounding_box.extend(range_data.sector.end_points[0].head<2>());
    bounding_box.extend(range_data.sector.end_points[1].head<2>());
  }
...
void CastRays(const sensor::RangeData& range_data,
  ...
  if (!range_data.sector.end_points.empty()) {
    ...
    for (const Eigen::Array2i &xy_index :
      ...
      if (cos > range_data.sector.max_cos || cos < -range_data.sector.min_cos  ) {
