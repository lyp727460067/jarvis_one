...
void Alignment::solveGyroscopeBias(map<double, ImageFrame> &all_image_frame,
  ...
  for (frame_i = all_image_frame.begin();
       next(frame_i) != all_image_frame.end(); ++frame_i) {
    frame_j = next(frame_i);
    frame_j->second.pre_integration->repropagate(Eigen::Vector3d::Zero(),
                                                 Bgs[0]);
  }
}
...
void Alignment::RefineGravity(map<double, ImageFrame> &all_image_frame,
...
bool Alignment::LinearAlignment(map<double, ImageFrame> &all_image_frame,
                                Eigen::Vector3d &g, Eigen::VectorXd &x) {
  int all_frame_count = all_image_frame.size();
...
bool Alignment::VisualIMUAlignment(
