...
/// 68k pixels / ms / GHz.
///
template <int border=3>
void fastDetect(const int width, const int height,const  cv::Mat &image,
                std::vector<std::vector<uint8_t>>& out, int threshold,const cv::Mat&mask) {
    
uint8x16_t vthreshold = vdupq_n_u8(threshold);
uint8x16_t zero = vdupq_n_u8(0);
uint8x16_t u0x40u = vdupq_n_u8(0x40u);
uint8x16_t u0x20u = vdupq_n_u8(0x20u);
uint8x16_t u0x10u = vdupq_n_u8(0x10u);
uint8x16_t u0x08u = vdupq_n_u8(0x08u);
uint8x16_t u0x04u = vdupq_n_u8(0x04u);
uint8x16_t u0x02u = vdupq_n_u8(0x02u);
uint8x16_t u0x01u = vdupq_n_u8(0x01u);
uint8x16_t u0x80u = vdupq_n_u8(0x80u);
//  jarvis::estimator::TicToc t_t;
  for (int y = border; y < height - border; y += 1) {
    // jarvis::estimator::TicToc t_t1;
    for (int x = border; x < width - border; x += 16) {
      uint8x16_t mask_d = vld1q_u8(mask.ptr(y - 3) + (x + 0));
      mask_d = vcgeq_u8(mask_d, u0x80u);
      mask_d = vtstq_u8(mask_d, mask_d);
      // mask_d = vcntq_u8(mask_d);
      uint64_t mask_value[2];
      vst1q_u8(reinterpret_cast<uint8_t *>(&mask_value), mask_d);
      // LOG(INFO)<<std::hex<<mask_value[0];
      if ((mask_value[0] || mask_value[1]) == 0) continue;
      //
      uint8x16_t c = vld1q_u8(image.ptr(y+0)+x);
      uint8x16_t light = vqaddq_u8(c, vthreshold);
      uint8x16_t dark = vqsubq_u8(c, vthreshold);

      uint8x16_t test = vld1q_u8(image.ptr(y-3)+x-1);
      uint8x16_t d0 = vcgeq_u8(test, dark);
      uint8x16_t l0 = vcleq_u8(test, light);

      test = vld1q_u8(image.ptr(y-3)+(x+0));
      d0 = vbslq_u8(u0x40u, vcgeq_u8(test, dark), d0);
      l0 = vbslq_u8(u0x40u, vcleq_u8(test, light), l0);

      test = vld1q_u8(image.ptr(y-3)+x+1);
      d0 = vbslq_u8(u0x20u, vcgeq_u8(test, dark), d0);
      l0 = vbslq_u8(u0x20u, vcleq_u8(test, light), l0);

      test = vld1q_u8(image.ptr(y-2)+x+2);
      d0 = vbslq_u8(u0x10u, vcgeq_u8(test, dark), d0);
      l0 = vbslq_u8(u0x10u, vcleq_u8(test, light), l0);

      test = vld1q_u8(image.ptr(y-1)+x+3);
      d0 = vbslq_u8(u0x08u, vcgeq_u8(test, dark), d0);
      l0 = vbslq_u8(u0x08u, vcleq_u8(test, light), l0);
      
      test = vld1q_u8(image.ptr(y+0)+x+3);
      d0 = vbslq_u8(u0x04u, vcgeq_u8(test, dark), d0);
      l0 = vbslq_u8(u0x04u, vcleq_u8(test, light), l0);
      
      test = vld1q_u8(image.ptr(y+1)+x+3);
      d0 = vbslq_u8(u0x02u, vcgeq_u8(test, dark), d0);
      l0 = vbslq_u8(u0x02u, vcleq_u8(test, light), l0);
      
      test = vld1q_u8(image.ptr(y+2)+x+2);
      d0 = vbslq_u8(u0x01u, vcgeq_u8(test, dark), d0);
      l0 = vbslq_u8(u0x01u, vcleq_u8(test, light), l0);
      ...
      test = vld1q_u8(image.ptr(y+3)+x+0);
      d1 = vbslq_u8(u0x40u, vcgeq_u8(test, dark), d1);
      l1 = vbslq_u8(u0x40u, vcleq_u8(test, light), l1);

      test = vld1q_u8(image.ptr(y+3)+x-1);
      d1 = vbslq_u8(u0x20u, vcgeq_u8(test, dark), d1);
      l1 = vbslq_u8(u0x20u, vcleq_u8(test, light), l1);

      test = vld1q_u8(image.ptr(y+2)+x-2);
      d1 = vbslq_u8(u0x10u, vcgeq_u8(test, dark), d1);
      l1 = vbslq_u8(u0x10u, vcleq_u8(test, light), l1);

      test = vld1q_u8(image.ptr(y+1)+x-3);
      d1 = vbslq_u8(u0x08u, vcgeq_u8(test, dark), d1);
      l1 = vbslq_u8(u0x08u, vcleq_u8(test, light), l1);

      test = vld1q_u8(image.ptr(y + 0) + x - 3);
      d1 = vbslq_u8(u0x04u, vcgeq_u8(test, dark), d1);
      l1 = vbslq_u8(u0x04u, vcleq_u8(test, light), l1);
      
      test = vld1q_u8(image.ptr(y-1)+x-3);
      d1 = vbslq_u8(u0x02u, vcgeq_u8(test, dark), d1);
      l1 = vbslq_u8(u0x02u, vcleq_u8(test, light), l1);
      
      test = vld1q_u8(image.ptr(y-2)+x-2);
      d1 = vbslq_u8(u0x01u, vcgeq_u8(test, dark), d1);
      l1 = vbslq_u8(u0x01u, vcleq_u8(test, light), l1);
      ...
      // asm("vceq.i8  q0, q0, #0 \n\t" :
      //        [val] "+w" (testLo));
      // testLo    = vcntq_u8(testLo);
      testLo = vceqq_u8(testLo,zero);
      
      uint8x16_t cntHi = vclzq_u8(t1);
      uint8x16_t testHi = t0 << (cntHi - 1);
      // asm("vceq.i8  q0, q0, #0" : [val] "+w" (testHi));
      // testHi= vcntq_u8(testHi);
      testHi= vceqq_u8(testHi,zero);
    ...
    // VLOG(10) << "fastDetect : " << t_t1.toc() << " ms";
    if (width % 16 != 0) {
      out[y][width  ] = 0;
      out[y][width+1] = 0;
    }
  }
  // VLOG(10) << "fastDetect : " << t_t.toc() << " ms";
}
...
void fastExtract(const int width, const int height,
  ...
  for (int y = border; y < height - border; y += 2) {
    ...
    for (int x = border; x < width - border; x += 2) {
      if (*mask.ptr(y, x) < 127 || *mask.ptr(y, x + 1) < 127 ||
          *mask.ptr(y + 1, x + 1) < 127 || *mask.ptr(y + 1, x) < 127 ) {
        continue;
      }
