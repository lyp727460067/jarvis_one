...
//
//
struct RecordFramData {
  uint32_t lenth;
  uint64_t time_stampe;
  uint64_t inc_id;
  uint8_t* data;
};
//
namespace jarvis_pic {
  class DataRecord {
   public:
    ...
    void AddVioData(jarvis::common::Time& time,
                    const jarvis::transform::Rigid3d& pose, bool slipe);
    void AddAtTimeFram(const uint64_t &time) ;
   private:
    void Run();
    void CreateDataDir();
    bool record_;
