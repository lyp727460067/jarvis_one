...
Eigen::Vector2d EquidistantCamera::DistortImage(
    const Eigen::Vector2d& uv) const {
  double x = (uv.x() - mParameters.u0()) / mParameters.mu();
  double y = (uv.y() - mParameters.v0()) / mParameters.mv();
  Eigen::Vector2d r;
  // undistToPlane(Eigen::Vector2d(x, y), r);
  spaceToPlane(Eigen::Vector3d(x, y, 1), r);
  return r;
}
/**
