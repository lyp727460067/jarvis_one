...
class ImuZeroVelocityDetect : public ZeroVelocityDetect {
 ...
 protected:
  ...
  const std::deque<sensor::ImuData>& data_base_;
};
//
struct StateType {
...
class UpdataZeroVelocity {
 public:
  ...
  void AddImageKeyPoints(const common::Time& time,
                         const ImageFeatureTrackerData& points);
  std::vector<uint64_t> GetOutlierPointClassId();

  const std::deque<sensor::ImuData>& getImuData() const { return imu_datas_; }
