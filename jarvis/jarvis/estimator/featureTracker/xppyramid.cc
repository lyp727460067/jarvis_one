...
#ifdef __ARM_NEON__
inline int32x4_t compute_bilinear_interpolation(
    const int16x8_t& top_row, const int16x8_t& bottom_row, const int16x4_t& w00,
    const int16x4_t& w01, const int16x4_t& w10, const int16x4_t& w11,
  ...
  return vqrshlq_s32(vmull_s16(px00, w00) + vmull_s16(px01, w01) +
                         vmull_s16(px10, w10) + vmull_s16(px11, w11),
                     shift);
}
#endif
...
void calcSharrDeriv(const cv::Mat& src, cv::Mat* dst) {
  ...
  for (y = 0; y < rows; ++y) {
    ...
    for (; x <= colsn - 8; x += 8) {
      __m128i s0 =
          _mm_loadu_si128(reinterpret_cast<const __m128i*>(trow0 + x - cn));
      __m128i s1 =
          _mm_loadu_si128(reinterpret_cast<const __m128i*>(trow0 + x + cn));
      __m128i s2 =
          _mm_loadu_si128(reinterpret_cast<const __m128i*>(trow1 + x - cn));
...
XPTrackerInvoker::XPTrackerInvoker(
    ...
    std::vector<bool>* _status, std::vector<float>* _err, Size _winSize,
    TermCriteria _criteria, int _level, int _maxLevel, int _start_level,
...
void XPTrackerInvoker::compute_covariance_matrix_and_update_patch(
  ...
  __m128 qA11 = _mm_setzero_ps(), qA12 = _mm_setzero_ps(),
         qA22 = _mm_setzero_ps();
#elif __ARM_NEON__
  ...
  const int32x4_t q11 =
      vdupq_n_s32(static_cast<int32_t>(5 - inter_param.w_bits));
  ...
  for (y = 0; y < winSize.height; y++) {
    ...
    const deriv_type* dsrc_row0 =
        derivI.ptr<deriv_type>(y + iprevPt.y) + iprevPt.x * cn2;
    const deriv_type* dsrc_row1 =
        derivI.ptr<deriv_type>(y + iprevPt.y + 1) + iprevPt.x * cn2;
    ...
    x = 0;
#if CV_SSE2
    for (; x <= winSize.width * cn - 4;
         x += 4, dsrc_row0 += 4 * 2, dsrc_row1 += 4 * 2, dIptr += 4 * 2) {
      __m128i v00, v01, v10, v11, t0, t1;

      v00 = _mm_unpacklo_epi8(
          _mm_cvtsi32_si128(*reinterpret_cast<const int*>(src_row0 + x)), z);
      v01 = _mm_unpacklo_epi8(
          _mm_cvtsi32_si128(*reinterpret_cast<const int*>(src_row0 + x + cn)),
          z);
      v10 = _mm_unpacklo_epi8(
          _mm_cvtsi32_si128(*reinterpret_cast<const int*>(src_row1 + x)), z);
      v11 = _mm_unpacklo_epi8(
          _mm_cvtsi32_si128(*reinterpret_cast<const int*>(src_row1 + x + cn)),
          z);

      t0 = _mm_add_epi32(_mm_madd_epi16(_mm_unpacklo_epi16(v00, v01), qw0),
                         _mm_madd_epi16(_mm_unpacklo_epi16(v10, v11), qw1));
      t0 = _mm_srai_epi32(_mm_add_epi32(t0, qdelta), inter_param.w_bits - 5);
      _mm_storel_epi64(reinterpret_cast<__m128i*>(Iptr + x),
                       _mm_packs_epi32(t0, t0));
    ...
    for (; x <= winSize.width * cn - 4;
      ...
      int32x4_t q5 = compute_bilinear_interpolation(
          ...
          vreinterpretq_s16_u16(vmovl_u8(vld1_u8(&src_row1[x]))), d26, d27, d28,
          d29, q11);
      ...
      int32x4_t q4 =
          vaddq_s32(vmull_s16(d0d1.val[0], d26), vmull_s16(d2d3.val[0], d27));
      int32x4_t q6 =
          vaddq_s32(vmull_s16(d0d1.val[1], d26), vmull_s16(d2d3.val[1], d27));

      int32x4_t q7 =
          vaddq_s32(vmull_s16(d4d5.val[0], d28), vmull_s16(d6d7.val[0], d29));
      int32x4_t q8 =
          vaddq_s32(vmull_s16(d4d5.val[1], d28), vmull_s16(d6d7.val[1], d29));

      q4 = vqrshlq_s32(vaddq_s32(q4, q7), q12);
      q6 = vqrshlq_s32(vaddq_s32(q6, q8), q12);

      int16x4x2_t d8d12 = {vmovn_s32(q4), vmovn_s32(q6)};
    ...
    for (; x < winSize.width; ++x, dsrc_row0 += 2, dsrc_row1 += 2, dIptr += 2) {
      int ival = CV_DESCALE(src_row0[x] * inter_param.iw00 +
                                src_row0[x + 1] * inter_param.iw01 +
                                src_row1[x] * inter_param.iw10 +
                                src_row1[x + 1] * inter_param.iw11,
                            inter_param.w_bits - 5);
      int ixval = CV_DESCALE(
          dsrc_row0[0] * inter_param.iw00 + dsrc_row0[2] * inter_param.iw01 +
              dsrc_row1[0] * inter_param.iw10 + dsrc_row1[2] * inter_param.iw11,
          inter_param.w_bits);
      int iyval = CV_DESCALE(
          dsrc_row0[1] * inter_param.iw00 + dsrc_row0[3] * inter_param.iw01 +
...
#elif __ARM_NEON__
  iA11 += static_cast<float>(vgetq_lane_s32(nA11, 0) + vgetq_lane_s32(nA11, 1) +
                             vgetq_lane_s32(nA11, 2) + vgetq_lane_s32(nA11, 3));
  iA12 += static_cast<float>(vgetq_lane_s32(nA12, 0) + vgetq_lane_s32(nA12, 1) +
                             vgetq_lane_s32(nA12, 2) + vgetq_lane_s32(nA12, 3));
  iA22 += static_cast<float>(vgetq_lane_s32(nA22, 0) + vgetq_lane_s32(nA22, 1) +
                             vgetq_lane_s32(nA22, 2) + vgetq_lane_s32(nA22, 3));
#endif
  ...
  *(A_ptr + 2) = A22;
  *(A_ptr + 3) =
      (A22 + A11 - std::sqrt((A11 - A22) * (A11 - A22) + 4.f * A12 * A12)) /
...
void XPTrackerInvoker::operator()(const Range& range) const {
  ...
  for (int ptidx = range.start; ptidx < range.end; ++ptidx) {
    ...
    DBG_XP_PYRAMID("ft_id: " << prevPts[ptidx].class_id
                             << " cur level = " << level);
...
#endif
    XPKeyPointRepo* kp_repo = &(prevPts[ptidx].keypoint_repo->pyramids[level]);
    ...
    if (prevPts[ptidx].need_to_update_repo) {
      compute_covariance_matrix_and_update_patch(
          iprevPt, InterpolationParam(iw00, iw01, iw10, iw11), &IWinBuf,
          &derivIWinBuf, kp_repo->covariance_maxtrix);
    }
    ...
    if (get_min_eigenvals) err[ptidx] = static_cast<float>(minEig);
    ...
    for (j = 0; j < max_iterations; ++j) {
      ...
      for (y = 0; y < wh; y++) {
        ...
        for (; x <= winSize.width - 4; x += 4, dIptr += 8) {
          ...
          // do interpolation
          int32x4_t q2 = compute_bilinear_interpolation(q0, q1, d26_2, d27_2,
                                                        d28_2, d29_2, q11);
        ...
        // loading x and y gradient
        q3 = vld2_s16(dIptr);
        q2 = vsetq_lane_s32(0, q2,
                            3);  // IMPORTANT: set lane3 to 0 for win_size 7
        // accumulate
        nb1 = vmlaq_s32(nb1, q2, vmovl_s16(q3.val[0]));
        nb2 = vmlaq_s32(nb2, q2, vmovl_s16(q3.val[1]));
        x += 3;
      ...
      ib2 += bbuf[1] + bbuf[3];
#elif __ARM_NEON__
      ib1 += vgetq_lane_s32(nb1, 0) + vgetq_lane_s32(nb1, 1) +
             vgetq_lane_s32(nb1, 2) + vgetq_lane_s32(nb1, 3);
      ib2 += vgetq_lane_s32(nb2, 0) + vgetq_lane_s32(nb2, 1) +
             vgetq_lane_s32(nb2, 2) + vgetq_lane_s32(nb2, 3);
#endif
      ...
      if (delta.ddot(delta) <= criteria.epsilon) {
        DBG_XP_PYRAMID("break delta < epsilon at itr = " << j << " nextPt = "
                                                         << nextPts[ptidx]);
        break;
      }
      if (std::abs(delta.x + prevDelta.x) < 0.01 &&
        ...
        DBG_XP_PYRAMID("break terminates at itr = " << j << " nextPt = "
                                                    << nextPts[ptidx]);
        break;
      }
    ...
    if (status[ptidx] && level == start_level &&
      ...
      // TODO(yanghongtian) : supportint patch here, update need_to_update_repo
      // flag. for now, need_to_update_repo is always true.
      ...
      for (y = 0; y < winSize.height; y++) {
        const uchar* Jptr =
            J.ptr() + (y + inextPoint.y) * stepJ + inextPoint.x * cn;
        const deriv_type* Iptr = IWinBuf.ptr<deriv_type>(y);
...
void XPcalcOpticalFlowPyrLK(const std::vector<cv::Mat>& _prevPyramids,
  ...
  for (level = _max_level; level >= _start_level; --level) {
    ...
    Mat derivI(imgSize.height, imgSize.width,
               CV_MAKETYPE(DataType<deriv_type>::depth, 2),
    ...
    XPTrackerInvoker(_prevPyramids[level], derivI, _nextPyramids[level],
