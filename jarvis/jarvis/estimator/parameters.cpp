...
void readParameters(std::string config_file) {
  ...
  if (ESTIMATE_EXTRINSIC == 2) {
    ...
    if (ESTIMATE_EXTRINSIC == 0) LOG(WARNING) << " fix extrinsic param ";
    ;
  ...
  if (ESTIMATE_TD)
    LOG(WARNING) << "Unsynchronized sensors, online estimate time offset, "
                    "initial td: "
                 << TD;
  else
    LOG(WARNING) << "Synchronized sensors, fix time offset: " << TD;
